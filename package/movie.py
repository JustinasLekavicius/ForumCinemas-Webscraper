...
from package import news
import sqlite3

class Movie:

    def __init__(self, title, foreign_title, director, age_rating, duration, is_3d, image, genre):
        self.title = title
    ...
    def __str__(self):
        return "Movie title: ", self.title, " director: ", self.director, ", age rating: ", self.age_rating, "duration in minutes: ", self.duration, ", the movie is in 3D: ", self.is_3d, ", image link: ", self.image, ", genre: ", self.genre


# A new movie is added to the movies.json file.
def add_movie(added_title, added_foreign_title, added_director, added_age_rating, added_duration, added_is_3d, added_image, added_genre):
    ...
    director VARCHAR(300), age_rating INTEGER, duration VARCHAR(10), is_3d BOOLEAN, image VARCHAR(200), genre VARCHAR(300))''')
    ...
    for item in movie_file:
        m = Movie(movie_file[item]["title"], movie_file[item]["foreign_title"], movie_file[item]["director"],
                  ...
                  movie_file[item]["duration"], movie_file[item]["is_3d"], movie_file[item]["image"], movie_file[item]["genre"])
    ...
    if added_is_3d == "false" or added_is_3d == "False" or added_is_3d == "no" or added_is_3d == "No":
        added_is_3d = False
    m = Movie(added_title, added_foreign_title, added_director, added_age_rating, added_duration, added_is_3d, added_image, added_genre)
    ...
    print("Movie added! Title:", m.movie_name(), ", foreign title: ", m.movie_foreign_title(), ", director: ",
          ...
          ", is in 3D: ", m.movie_is_3d(), ", image link: ", m.movie_image(), ", genre: ", m.movie_genre())
    c.execute('SELECT * FROM movies WHERE (title=? AND foreign_title=? AND director=? AND age_rating=? AND duration=? AND is_3d=? AND genre=?)',
              (m.movie_name(), m.movie_foreign_title(), m.movie_director(), m.movie_age_rating(), m.movie_duration(),
                  m.movie_is_3d(), m.movie_genre()))
    entry = c.fetchone()
    if entry is None:
        conn.execute(
            'INSERT INTO movies (title, foreign_title, director, age_rating, duration, is_3d, image, genre) VALUES (?,?,?,?,?,?,?,?)', (
            m.movie_name(), m.movie_foreign_title(), m.movie_director(), m.movie_age_rating(), m.movie_duration(),
            m.movie_is_3d(), m.movie_image(), m.movie_genre()))
    else:
        pass
    conn.commit()
    conn.close()
