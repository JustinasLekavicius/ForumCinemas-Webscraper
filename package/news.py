...
def add_news(added_title, added_description, added_link):
    ...
    c = conn.cursor()
    c.execute('''CREATE TABLE IF NOT EXISTS news (id INTEGER PRIMARY KEY AUTOINCREMENT, title VARCHAR(1000), description VARCHAR(10000),
        link VARCHAR(300))''')
    ...
    print("News found and added! Title:", n.news_name(), ", description: ", n.news_description(), ", link: ",
          n.news_link())
    c.execute('SELECT * FROM news WHERE (title=? AND description=? AND link=?)', (n.news_name(), n.news_description(),n.news_link()))
    entry = c.fetchone()
    if entry is None:
        conn.execute(
            'INSERT INTO news (title, description, link) VALUES (?,?,?)',
            (n.news_name(), n.news_description(), n.news_link()))
    else:
        pass
    conn.commit()
    conn.close()
